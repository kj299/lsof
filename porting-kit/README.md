...
| `harnesses/gate-mutation/mutate_gates.py` | break each gate's verdict on purpose and require its self-test to go red; audits its own table for uncovered harnesses. Every decision in a verdict function is also forced both ways; one the self-test misses fails unless `unpinned.jsonl` names it with a reason, and that ledger only shrinks (LESSONS #068) | **CI** |
