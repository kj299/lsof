# winlsof — scoping a Linux backend
...
because **the C `lsof` cannot run on Windows**, so winlsof is compared against
...
1. **The name.** `winlsof` stops being accurate the moment Linux lands. Options:
   rename the workspace (churns every doc and the release-tag prefix), or keep
   `winlsof` as the *Windows binary's* name under a differently-named workspace.
   Decide **before** L0, not after.
2. **Coverage matrix shape.** The matrix and feature inventory are currently
   Windows-shaped (`feature-inventory-winlsof.toml`, 118 waivers reading
...
2. A second backend churns the coverage matrix, the naming, and the
   "winlsof = Windows" framing in every document. That is much cleaner to do
