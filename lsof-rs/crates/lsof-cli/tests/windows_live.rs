...
#[test]
fn version_and_help() {
    assert!(lsof(&["-v"]).contains("lsof-rs"), "version banner missing");
    let help = lsof(&["-h"]);
...
fn lists_its_own_open_file() {
    ...
    path.push(format!("lsof_rs_live_{pid}.dat"));

    let mut file = std::fs::File::create(&path).expect("create temp file");
    file.write_all(b"lsof-rs live test").unwrap();
    file.flush().unwrap();
    ...
    let needle = format!("lsof_rs_live_{pid}");
    assert!(
