...
/// setting the `LSOF_RS_TRACE` environment variable to anything; writes one
...
///
/// `WINLSOF_TRACE` is still honored. It is the name this shipped under from
/// v0.1.0 through v1.0.1 and it appears in the troubleshooting docs of those
/// releases, so a runbook written against a shipped binary keeps working; the
/// rename would otherwise silently turn tracing off exactly when someone is
/// reaching for it.
pub fn trace(msg: &str) {
    static ENABLED: OnceLock<bool> = OnceLock::new();
    if *ENABLED.get_or_init(|| {
        std::env::var_os("LSOF_RS_TRACE").is_some() || std::env::var_os("WINLSOF_TRACE").is_some()
    }) {
        eprintln!("[lsof-rs] {msg}");
    }
}
