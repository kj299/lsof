...
- The release drops `--prerelease` in
  [`lsof-rs-release.yml`](../../.github/workflows/lsof-rs-release.yml) (it is
...
| 5 | **Release-candidate field validation**: the **exact release artifact** (downloaded `lsof.exe`, not a local build) passes the full smoke suite (62 cases today) on real Windows 11 hardware in **both** privilege modes — the per-release checkpoint below — with zero FAIL and zero hangs. | ⬜ per release — v0.4.0 ✅; v1.0.0 ❌; **v1.0.1 ✅** (see log) |
...
So lsof-rs ships **unsigned + SHA-256** as a deliberate, privacy-conscious
...
the `lsof-rs/` subtree to an OSI license to unblock the free SignPath Foundation
...
Corroborating detail: the fuzzer independently derived lsof-rs's own option
...
not lsof-rs; a green result would prove an approximation, not the real
...
.\Invoke-LsofRsSmokeTest.ps1 -Binary <path-to-downloaded>\lsof.exe
