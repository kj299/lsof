# lsof-rs — live Windows validation plan

This plan validates the `lsof-rs` (`lsof.exe`) build end-to-end on a real
...
**Pass:** the PID lsof-rs reports for port 8765 equals
...
```
powershell 4180 DOMAIN\alice  784r REG C: 3 196610 C:\Users\alice\AppData\Local\Temp\lsof_rs_test.txt
```
...
& $lsof $path
# T13 — directory form
& $lsof +D $env:TEMP 2>$null | Select-String 'lsof_rs_test'
...
```
COMMAND     PID USER         FD  TYPE DEVICE SIZE/OFF NODE NAME
powershell 4180 DOMAIN\alice unk REG  C:                    C:\Users\alice\AppData\Local\Temp\lsof_rs_test.txt
```
...
($mem | Where-Object { $_ -notin $mods }).Count    # expect 0 (all lsof-rs mem entries are real modules)
```
