# Code-signing lsof-rs release binaries — tracking doc
...
**Signing is not a 1.0 blocker and is not currently planned.** lsof-rs ships
...
relicense `lsof-rs/` to an OSI license to unblock the free SignPath Foundation
...
- **SignPath Foundation (free, OSS) blocked.** Requires an OSI-approved
  license; lsof-rs ships as `LicenseRef-lsof` (the custom lsof/Purdue license,
  permissive but not OSI). Relicensing `lsof-rs/` to MIT/Apache-2.0 would
...
`.github/workflows/lsof-rs-release.yml` signs `lsof.exe` before the SHA-256 is
...
- Update the README "Antivirus / Defender note" and the release-notes
  template in `lsof-rs-release.yml` (drop "the binary is unsigned"; keep
...
- **Microsoft Defender** flags the downloaded `lsof.exe` as a hacktool / PUA
  and refuses to launch it, because lsof-rs legitimately does what an
...
- [ ] Update `lsof-rs/README.md` "Antivirus / Defender note" once signing
  lands (remove the `Add-MpPreference` workaround, keep the SHA-256
  verification path).
- [ ] Drop the matching note from `.github/workflows/lsof-rs-release.yml`
...
- An MSIX / installer wrapper — lsof-rs is a single-file CLI; keep it that way.
