<#
.SYNOPSIS
    Quick, self-contained smoke test for a lsof-rs lsof.exe binary.
...
Write-Host "lsof-rs quick smoke test" -ForegroundColor Cyan
...
try {
    $r = Invoke-Bin @('-v')
    Test-That 'version banner'        ($r.Out -match 'lsof-rs') "exit=$($r.Exit)"
