...
| 30 | whole-host **peak RSS** is ~2.9x the C and grows with the host (9.8 MB against 29.1 MB at 1075 processes) | **OPEN — measured, gated against getting worse, not fixed** | architecture, not a bug in a code path: the C streams a row and forgets it, this port builds `Vec<Process>` with a `Vec<OpenFile>` each and hands the whole thing to selection. Closing it means making the pipeline streaming, which is a redesign of the `Backend` seam and not something P5 should do quietly. `differential/resource_gate.py` pins the current ratio at a 3.50x ceiling so it cannot drift further unnoticed. The `-i` half of the same cost WAS fixed (4.32x -> 1.41x at 1075 processes) because there the rows were not merely retained, they were built for a selection that could never keep them. |
