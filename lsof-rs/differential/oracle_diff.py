#!/usr/bin/env python3
"""Oracle-substitution differential for lsof-rs's socket view.
...
socket table as the oracle: it compares lsof-rs's ``-i`` output against
...
capture (``capture.ps1``) writes two artifacts -- lsof-rs's own output and the
...
EXIT_OK = 0
EXIT_DIVERGENCE = 1  # lsof-rs's set differs from the oracle's
...
def canon_remote(host, port):
    ...
    (and lsof-rs) omit the peer entirely. Treat both as "no remote".
    """
    if host is None:
        return "-"
...
def parse_lsof_rs_json(text):
    """Parse lsof-rs ``-i -J`` output. Socket rows carry a ``protocol`` key."""
...
def parse_lsof_rs_fields(text):
    """Parse lsof-rs ``-i -F`` output (newline-terminated token stream).
...
def diff(lsof_rs_rows, oracle_rows, scope_ports=None, scope_pid=None, ledger=None):
    """Multiset diff of lsof-rs vs oracle within scope.

    Returns (missing, extra, notes); each item is (representative_row, why, count).

    missing = in oracle, under-reported by lsof-rs.
    extra   = in lsof-rs, not backed by the oracle.
    ...
    """
    ledger = ledger or []
    w_rows = scoped(lsof_rs_rows, scope_ports, scope_pid)
    ...
    for k in sorted(set(wc) | set(oc), key=repr):
        delta = wc[k] - oc[k]
        if delta < 0:  # oracle has more copies -> lsof-rs under-reports
            row = orep[k]
            why = _suppressed(row, "missing", ledger)
            (notes if why else missing).append((row, why, -delta))
        elif delta > 0:  # lsof-rs has more copies -> over-reports
            row = wrep[k]
...
def _strip_banner(text):
    """Drop lsof-rs's non-Windows mock banner / any non-token preamble lines."""
    out = []
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="lsof-rs socket oracle-substitution differential")
    src = ap.add_mutually_exclusive_group(required=True)
    src.add_argument("--lsof-rs-json", help="file with lsof-rs `-i -J` output")
    src.add_argument("--lsof-rs-fields", help="file with lsof-rs `-i -F` output")
    ...
    try:
        if args.lsof_rs_json:
            lsof_rs_rows = parse_lsof_rs_json(_read(args.lsof_rs_json))
        else:
            lsof_rs_rows = parse_lsof_rs_fields(_read(args.lsof_rs_fields))
        oracle_rows = parse_oracle_json(_read(args.oracle))
    except (ValueError, json.JSONDecodeError) as exc:
        # Malformed/truncated capture (e.g. lsof-rs killed on the hang path) is
        # infrastructure breakage, not a socket-set divergence -- keep it a
        # distinct exit so CI triage doesn't read it as "lsof-rs is wrong".
    ...
    w_scoped = scoped(lsof_rs_rows, scope_ports, args.scope_pid)
    o_scoped = scoped(oracle_rows, scope_ports, args.scope_pid)
    print("lsof-rs rows: {0} ({1} in scope)   oracle rows: {2} ({3} in scope)   scope: pid={4} ports={5}".format(
        len(lsof_rs_rows), len(w_scoped), len(oracle_rows), len(o_scoped),
    ...
    # vacuous: an empty==empty "match" would hide a lsof-rs enumeration
    ...
    if len(w_scoped) < args.min_rows or len(o_scoped) < args.min_rows:
        print("INFRA: too few in-scope rows (lsof-rs={0}, oracle={1}, need >= {2}); "
              "capture likely empty/failed -- not a divergence verdict".format(
                  len(w_scoped), len(o_scoped), args.min_rows))
        return EXIT_INFRA

    missing, extra, notes = diff(lsof_rs_rows, oracle_rows, scope_ports, args.scope_pid, ledger)

    for row, why, count in notes:
        print("  LEDGERED  {0}  <- {1}".format(_fmt(row, count), why))
    for row, _why, count in missing:
        print("  MISSING   {0}   (oracle has it, lsof-rs does not)".format(_fmt(row, count)))
    for row, _why, count in extra:
        print("  EXTRA     {0}   (lsof-rs has it, oracle does not)".format(_fmt(row, count)))

    if missing or extra:
        print("DIFFERENTIAL FAILED: {0} missing, {1} extra ({2} ledgered)".format(
            len(missing), len(extra), len(notes)))
        return EXIT_DIVERGENCE
    print("DIFFERENTIAL OK: lsof-rs's socket set matches the OS oracle within scope "
          "({0} ledgered divergence(s))".format(len(notes)))
    return EXIT_OK


def _read(path):
    # The capture artifacts are UTF-8 (lsof-rs `-i -J` embeds every process's
